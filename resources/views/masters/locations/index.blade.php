...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-600">Kelola data lokasi aset</p>
        </div>
        <a href="{{ route('masters.locations.create') }}" class="inline-flex items-center px-4 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3 text-sm font-medium">
                        <div class="flex items-center space-x-2">
                            <a href="{{ route('masters.locations.edit', $item) }}" class="text-yellow-600 hover:text-yellow-900" title="Edit"><i class="fas fa-edit"></i></a>
                            <form action="{{ route('masters.locations.destroy', $item) }}" method="POST" onsubmit="return confirm('Hapus lokasi ini?')">
                                @csrf
                                @method('DELETE')
