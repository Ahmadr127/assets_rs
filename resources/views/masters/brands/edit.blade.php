...
<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-white shadow-sm border border-gray-200 rounded-lg p-6">
        <form action="{{ route('masters.brands.update', $brand) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            <x-form.input name="name" label="Nama" :value="$brand->name" required autofocus />
            <x-form.input name="code" label="Kode" :value="$brand->code" />
            <x-form.textarea name="description" label="Deskripsi" rows="4" :value="$brand->description" />
