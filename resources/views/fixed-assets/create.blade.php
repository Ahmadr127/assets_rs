...
@section('title', 'Tambah Fixed Asset')
...
<div class="w-full">
    ...
    <div class="flex items-center justify-between mb-3">
        <div>
            <h1 class="text-xl font-bold text-gray-900">Tambah Fixed Asset</h1>
            <p class="text-sm text-gray-600">Tambahkan aset tetap baru ke sistem</p>
        </div>
    ...
    <div class="bg-white shadow border border-gray-200 rounded">
        <form action="{{ route('fixed-assets.store') }}" method="POST" class="p-4">
            @csrf
            
            <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-3">
                    <!-- Tipe Fixed Asset (Master with Suggest/Create) -->
                    <div>
                        <x-form.async-select name="asset_type_id" entity="types" label="Tipe Fixed Asset" :required="true" placeholder="Contoh : Peralatan Komputer " />
                    </div>
                ...
                <!-- Lokasi -->
                <div>
                    <x-form.async-select name="location_id" entity="locations" label="Lokasi" :required="true" placeholder="Contoh: Unit IT, Sekretaris, Ruang Rapat" />
                </div>

                <!-- Status -->
                <div>
                    <x-form.combobox name="status_id" label="Status" :options="$statuses" :required="true" placeholder="Pilih Status..." :searchable="true" :clearable="false" />
                </div>

                <!-- Kondisi -->
                <div>
                    <x-form.async-select name="condition_id" entity="conditions" label="Kondisi" :required="true" placeholder="Contoh: Baik, Rusak Ringan, Rusak Berat" />
                </div>

                <!-- Vendor -->
                <div>
                    <x-form.async-select name="vendor_id" entity="vendors" label="Vendor" placeholder="Contoh: PT ABC, CV XYZ, Toko Komputer" />
                </div>

                <!-- Brand -->
                <div>
                    <x-form.async-select name="brand_id" entity="brands" label="Brand" placeholder="Contoh: HP, Dell, Canon, Samsung" />
                </div>
            ...
            <div class="mt-3">
                <div class="flex items-center">
                    <input type="checkbox" 
                           ...
                           value="1"
                           {{ old('harus_dicek_fisik', true) ? 'checked' : '' }}
