...
<div class="max-w-4xl mx-auto">
    <div class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Tambah Fixed Asset</h1>
                <p class="mt-1 text-sm text-gray-600">Tambahkan aset tetap baru ke sistem</p>
            </div>
        ...
        <div class="bg-white shadow-sm border border-gray-200 rounded-lg">
            <form action="{{ route('fixed-assets.store') }}" method="POST" class="p-6 space-y-6">
                @csrf
                    ...
                    <div>
                        ...
                        <select id="kondisi" 
                            ...
                            @foreach(\App\Models\FixedAsset::getConditionOptions() as $value => $label)
                                <option value="{{ $value }}" {{ old('kondisi') == $value ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                        @error('kondisi')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
